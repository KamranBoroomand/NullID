...
- The inner workflow package remains plain JSON. Wrapping the exported file in `NULLID:ENC:2` adds confidentiality and AES-GCM integrity for that outer file transport, not public-key authenticity.
- Receivers can inspect exported packages locally in the app's `Verify Package` surface or with CLI `package-inspect`.

## CLI Relationship

The CLI does not expose a full interactive Safe Share wizard.
