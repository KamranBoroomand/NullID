...
- `NULLID:ENC:2` remains the encryption envelope format
...
In the supported browser/PWA and CLI surfaces, the contract is emitted by:
...
- The current inner workflow `trust.encryptedPayload.method` is `none`. `NULLID:ENC:2` is an optional outer envelope for the exported file, not inner workflow-package encryption.
...
This contract preserves current behavior by extending rather than replacing:
...
Receiver verification, Safe Share Assistant exports, and first-class Incident Workflow exports now all sit on the same shared contract. Later release scopes can keep expanding producer and receiver coverage without inventing a second package schema.
