...
- profile snapshots
- vault snapshots
- `NULLID:ENC:2` envelopes as inspectable containers, with optional local decryption
...
2. Paste a JSON artifact or `NULLID:ENC:2` envelope, or load a local file.
...
- `Malformed`: the input could not be parsed as supported JSON or as a `NULLID:ENC:2` envelope.
...
- that the inner workflow JSON was encrypted; only an outer `NULLID:ENC:2` envelope proves encrypted file transport
