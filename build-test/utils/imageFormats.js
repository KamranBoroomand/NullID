const outputMimes = ["image/png", "image/jpeg", "image/webp", "image/avif"];
const formatRows = [
...
export async function probeCanvasEncodeSupport() {
    const checks = await Promise.all(outputMimes.map(async (mime) => [mime, await canEncodeMime(mime)]));
    return Object.fromEntries(checks);
}
export async function probeImageFormatDiagnostics() {
    const outputSupport = await probeCanvasEncodeSupport();
    const fallbackClean = outputSupport["image/png"] || outputSupport["image/jpeg"];
    const rows = await Promise.all(formatRows.map(async (format) => {
        const decodeProbe = await probeDecodeSupport(format.mime);
        const decode = normalizeDecodeState(format.mime, decodeProbe);
        const encode = format.mime in outputSupport ? (outputSupport[format.mime] ? "yes" : "no") : "n/a";
        const cleanExport = decode === "no" ? "no" : fallbackClean ? "yes" : "no";
        return {
            ...format,
            decode,
            encode: encode,
...
export function chooseExportMime(inputMime, outputSupport) {
    const normalized = (inputMime || "").toLowerCase();
    if (normalized === "image/png" && outputSupport["image/png"])
        return "image/png";
    if (normalized === "image/webp" && outputSupport["image/webp"])
        return "image/webp";
    if (normalized === "image/avif" && outputSupport["image/avif"])
        return "image/avif";
    if (normalized === "image/jpeg" && outputSupport["image/jpeg"])
        return "image/jpeg";
    if (outputSupport["image/png"])
        return "image/png";
    if (outputSupport["image/jpeg"])
        return "image/jpeg";
    if (outputSupport["image/webp"])
        return "image/webp";
    if (outputSupport["image/avif"])
        return "image/avif";
    return "image/png";
}
function normalizeDecodeState(mime, raw) {
    if (raw === true)
        return "yes";
    if (raw === false)
        return "no";
...
async function probeDecodeSupport(mime) {
...
async function canEncodeMime(mime) {
