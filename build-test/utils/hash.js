...
function createProgressReporter(onProgress) {
    ...
    return (percent) => {
        ...
        if (clamped === lastPercent)
            return;
        if (clamped !== 100 && now - lastReport < PROGRESS_THROTTLE_MS)
            return;
        lastPercent = clamped;
        lastReport = now;
        onProgress(clamped);
...
async function yieldToMain(lastYield) {
...
async function finalizeHash(hasher, started, onProgress) {
...
export async function hashText(text, algorithm, options) {
...
export function normalizeHashInput(value) {
    if (!value)
        return "";
...
export const expectedHashLengths = {
    "SHA-1": 40,
    "SHA-256": 64,
    "SHA-512": 128,
};
