...
function createAssessment(options) {
    ...
    if (options.algorithm === "argon2id") {
        ...
        if (memory < 65_536)
            warnings.push("Argon2 memory cost is below 64 MiB");
        if (passes < 3)
            warnings.push("Argon2 passes below recommended minimum (3)");
    ...
    if (options.algorithm === "pbkdf2-sha256") {
        ...
        if (iterations < 300_000)
            warnings.push("PBKDF2 iterations below 300,000");
        return { safety: warnings.length > 0 ? "fair" : "strong", warnings };
    }
...
async function derivePbkdf2(secret, salt, iterations) {
    ...
    }, keyMaterial, DEFAULT_DERIVED_BITS);
