...
export const PASSWORD_HASH_ALGORITHMS = ["argon2id", "pbkdf2-sha256", "sha512", "sha256"];
...
function createAssessment(options) {
    ...
    if (options.algorithm === "argon2id") {
        ...
        if (memory < PASSWORD_HASH_LIMITS.argon2.memory.recommendedMin)
            warnings.push(PASSWORD_HASH_WARNINGS.argon2MemoryBelowRecommended);
        if (passes < PASSWORD_HASH_LIMITS.argon2.passes.recommendedMin)
            warnings.push(PASSWORD_HASH_WARNINGS.argon2PassesBelowRecommended);
    ...
    if (options.algorithm === "pbkdf2-sha256") {
        ...
        if (iterations < PASSWORD_HASH_LIMITS.pbkdf2.iterations.recommendedMin)
            warnings.push(PASSWORD_HASH_WARNINGS.pbkdf2IterationsBelowRecommended);
        return { safety: warnings.length > 0 ? "fair" : "strong", warnings };
    }
...
async function derivePbkdf2(secret, salt, iterations) {
    ...
    }, keyMaterial, PASSWORD_HASH_LIMITS.record.derivedBits);
    return new Uint8Array(bits);
}
async function deriveArgon2id(secret, salt, options) {
    const subtle = crypto.subtle;
    const keyMaterial = await subtle.importKey("raw-secret", toArrayBuffer(utf8ToBytes(secret)), "Argon2id", false, ["deriveBits"]);
...
function deriveShaDigest(secret, salt, algorithm) {
    const payload = concatBytes(salt, utf8ToBytes(secret));
    return algorithm === "sha512" ? sha512(payload) : sha256(payload);
}
export async function supportsArgon2id() {
    if (argon2SupportCache !== null)
...
export async function hashPassword(secret, options) {
    if (!secret) {
...
function validateSaltLength(salt) {
...
function validateDigestLength(algorithm, digest) {
...
function validateArgon2Params(memory, passes, parallelism) {
...
export function parsePasswordHash(encoded) {
    const argonMatch = encoded.match(new RegExp(`^\\$argon2id\\$v=${PASSWORD_HASH_LIMITS.record.argon2Version}\\$m=(\\d+),t=(\\d+),p=(\\d+)\\$(${PASSWORD_HASH_LIMITS.record.base64Segment})\\$(${PASSWORD_HASH_LIMITS.record.base64Segment})$`, "u"));
    if (argonMatch) {
    ...
    const pbkdf2Match = encoded.match(new RegExp(`^\\$pbkdf2-sha256\\$i=(\\d+)\\$(${PASSWORD_HASH_LIMITS.record.base64Segment})\\$(${PASSWORD_HASH_LIMITS.record.base64Segment})$`, "u"));
    if (pbkdf2Match) {
    ...
    const shaMatch = encoded.match(new RegExp(`^\\$(sha256|sha512)\\$s=(${PASSWORD_HASH_LIMITS.record.base64Segment})\\$(${PASSWORD_HASH_LIMITS.record.base64Segment})$`, "u"));
    if (shaMatch) {
...
export async function verifyPassword(secret, encoded) {
    if (!secret)
        return false;
...
export function assessPasswordHashChoice(options) {
