...
export function sanitizeKeyHint(value) {
    const normalized = (value ?? "").trim();
    return normalized ? normalized.slice(0, 64) : "";
}
export function rotateKeyHint(current, nextVersion) {
    const base = current.replace(/-v\d+$/i, "");
    return `${base || "hint"}-v${nextVersion}`;
}
export function upsertKeyHintProfile(profiles, nameRaw, hintRaw, nowIso = new Date().toISOString()) {
    const name = nameRaw.trim();
    const keyHint = sanitizeKeyHint(hintRaw);
    if (!name || !keyHint) {
        return { ok: false, message: "profile name + key hint required" };
    }
...
export function rotateProfileHint(profiles, id, nowIso = new Date().toISOString()) {
    const profile = profiles.find((entry) => entry.id === id);
    if (!profile) {
        return { ok: false, message: "key hint profile missing" };
    }
    ...
    return {
        ok: true,
        hint: nextHint,
        profiles: profiles.map((entry) => entry.id === id
            ? {
                ...entry,
                version: nextVersion,
                keyHint: nextHint,
...
export function removeProfileHint(profiles, id) {
    return profiles.filter((profile) => profile.id !== id);
}
export function readLegacyProfiles(storageKey) {
