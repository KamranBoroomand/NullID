import { decryptText, inspectEnvelope } from "./cryptoEnvelope.js";
...
export async function inspectReceivedArtifact(rawInput, options) {
...
function inspectEnvelopePayload(payload, options) {
    let meta;
    try {
...
async function inspectParsedOrMalformedAfterDecrypt(decrypted, envelope, options) {
    try {
...
async function inspectParsedArtifact(parsed, options) {
    if (looksLikeWorkflowArtifact(parsed)) {
        ...
        return {
            ...
            unverifiedChecks: verified.unverifiedChecks.concat(schema2SafeShareBundle
                ? [
...
function looksLikeVault(value) {
    return isRecord(value) && (value.kind === "vault" || value.vault !== undefined || value.notes !== undefined || value.meta !== undefined);
}
function isRecord(value) {
    return Boolean(value) && typeof value === "object" && !Array.isArray(value);
}
function overallArtifactStatus(state) {
    if (state === "verified" || state === "integrity-checked")
        return "verified";
    if (state === "mismatch")
        return "mismatch";
    return "unverified";
}
function summarizeWorkflowPolicy(policy) {
    if (!policy)
        return [];
    const facts = [{ label: "Policy type", value: policy.type }];
    if (policy.preset)
        facts.push({ label: "Preset", value: policy.preset });
    if (policy.packName)
        facts.push({ label: "Pack", value: policy.packName });
    if (policy.baseline)
        facts.push({ label: "Baseline", value: policy.baseline });
    if (policy.config) {
...
function summarizeWorkflowReport(report) {
    if (!report)
        return undefined;
    return {
...
function summarizeDescriptiveWorkflowMetadata(workflowPackage) {
    ...
    if (!workflowPackage.summary.title &&
        facts.length === 0 &&
        transforms.length === 0 &&
