...
import { PASSWORD_HASH_ALGORITHMS, assessPasswordHashChoice, hashPassword, parsePasswordHash, PASSWORD_HASH_DEFAULTS, PASSWORD_HASH_LIMITS, PASSWORD_HASH_MESSAGES, supportsArgon2id, verifyPassword, } from "../utils/passwordHashing.js";
const sharedSpec = JSON.parse(fs.readFileSync(path.resolve(process.cwd(), "src/utils/passwordHashingSpec.json"), "utf8"));
function encodeBase64(byteLength, fill) {
...
describe("password hashing", () => {
    ...
    it("accepts imported records with padded standard base64 fields", () => {
        const parsed = parsePasswordHash(`$sha256$s=${encodeBase64(PASSWORD_HASH_LIMITS.saltBytes.min, 0xff)}$${encodeBase64(PASSWORD_HASH_LIMITS.record.derivedBytes, 0x11)}`);
    ...
    it("rejects Argon2id cost parameters outside NullID bounds", () => {
        const salt = encodeBase64(PASSWORD_HASH_LIMITS.saltBytes.default, 0x01);
        const digest = encodeBase64(PASSWORD_HASH_LIMITS.record.derivedBytes, 0x22);
        const message = getErrorMessage(() => parsePasswordHash(`$argon2id$v=${PASSWORD_HASH_LIMITS.record.argon2Version}$m=999999,t=${PASSWORD_HASH_LIMITS.argon2.passes.default},p=${PASSWORD_HASH_LIMITS.argon2.parallelism.default}$${salt}$${digest}`));
