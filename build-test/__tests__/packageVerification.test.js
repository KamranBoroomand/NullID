...
describe("received artifact verification", () => {
    it("verifies current safe-share bundles through the shared workflow package contract", async () => {
        ...
        assert.equal(result.facts.some((fact) => fact.label === "Verification basis" && fact.value === "Embedded workflow package"), true);
        assert.equal(result.facts.some((fact) => fact.label === "Bundle schema" && fact.value === "2"), true);
    ...
    it("verifies Safe Share Assistant workflow packages and surfaces the workflow preset", async () => {
        ...
        assert.equal(result.facts.some((fact) => fact.label === "Workflow preset" && fact.value === "Support ticket / bug report"), false);
        assert.equal(result.descriptiveWorkflowMetadata?.facts.some((fact) => fact.label === "Workflow preset" && fact.value === "Support ticket / bug report"), true);
    ...
    it("treats malformed workflow package payloads as invalid instead of throwing", async () => {
        const result = await inspectReceivedArtifact(JSON.stringify({
            kind: "nullid-workflow-package",
            schemaVersion: 99,
            packageType: "bundle",
        }));
...
async function createSignedProfileSnapshot(signingPassphrase) {
    const exportedAt = "2026-03-17T09:00:00.000Z";
    const entries = { "nullid:theme": "light" };
...
async function createSignedVaultSnapshot(signingPassphrase) {
    const vault = buildVaultSnapshotFixture();
    const exportedAt = "2026-03-17T08:00:00.000Z";
    const { integrity, signature } = await createSnapshotIntegrity({
        schemaVersion: 2,
