...
import { createIncidentFileArtifactPackage, createIncidentTextArtifactPackage, createIncidentWorkflowPackage, } from "../utils/incidentWorkflow.js";
...
describe("incident workflow", () => {
    it("creates a composed incident package with flattened prepared artifacts and honest trust limits", async () => {
        ...
        assert.equal(incidentPackage.artifacts.some((artifact) => artifact.kind === "binary" && /Responder capture/i.test(artifact.label)), true);
        ...
        assert.equal(verified.artifactChecks.some((artifact) => artifact.kind === "binary" && artifact.status === "verified"), true);
    });
