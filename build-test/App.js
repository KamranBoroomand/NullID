...
function WorkspaceView({ active, onRegisterHashActions, onStatus, onOpenGuide }) {
    const { tr } = useI18n();
    return (_jsxs(Suspense, { fallback: _jsx("div", { className: "workspace-loading", children: tr("loading module...") }), children: [active === "hash" ? _jsx(HashView, { onRegisterActions: onRegisterHashActions, onStatus: onStatus, onOpenGuide: onOpenGuide }) : null, active === "share" ? _jsx(SafeShareView, { onOpenGuide: onOpenGuide }) : null, active === "incident" ? _jsx(IncidentWorkflowView, { onOpenGuide: onOpenGuide }) : null, active === "redact" ? _jsx(RedactView, { onOpenGuide: onOpenGuide }) : null, active === "sanitize" ? _jsx(SanitizeView, { onOpenGuide: onOpenGuide }) : null, active === "verify" ? _jsx(VerifyPackageView, { onOpenGuide: onOpenGuide }) : null, active === "meta" ? _jsx(MetaView, { onOpenGuide: onOpenGuide }) : null, active === "enc" ? _jsx(EncView, { onOpenGuide: onOpenGuide }) : null, active === "pw" ? _jsx(PwView, { onOpenGuide: onOpenGuide }) : null, active === "vault" ? _jsx(VaultView, { onOpenGuide: onOpenGuide }) : null, active === "selftest" ? _jsx(SelfTestView, { onOpenGuide: onOpenGuide }) : null, active === "guide" ? _jsx(GuideView, {}) : null] }));
}
function AppShell() {
    const { push } = useToast();
    ...
    const profileExportTrustState = !profileExportSign
        ? "unsigned"
        : profileExportPassphrase.trim()
            ? "verified"
            : "mismatch";
    const profileImportTrustState = useMemo(() => {
        if (!profileImportDescriptor?.signed)
            return "unsigned";
        if (!profileImportPassphrase.trim())
            return "mismatch";
        if (profileImportError && /verification|signature|mismatch|integrity/i.test(profileImportError))
            return "mismatch";
        return "verified";
    ...
    return (_jsxs("div", { className: `app-surface ${isCompact ? "is-compact" : ""}`, children: [_jsx("input", { ref: importProfileInputRef, type: "file", accept: "application/json", style: { position: "absolute", opacity: 0, width: 1, height: 1, pointerEvents: "none" }, tabIndex: -1, onChange: async (event) => {
                    ...
                    event.target.value = "";
                                    ...
                                    if (wipeError)
                                        setWipeError(null);
                                }, "aria-label": tr("Type WIPE to confirm"), placeholder: "WIPE", autoComplete: "off", disabled: wipeBusy })] }), wipeError ? _jsx("p", { className: "action-dialog-error", children: wipeError }) : null] }), _jsxs(ActionDialog, { open: profileExportOpen, title: tr("Export profile snapshot"), description: tr("Export local nullid:* settings as JSON. Optional HMAC metadata can be verified during import with the same passphrase."), confirmLabel: tr("export profile"), onCancel: closeProfileExportDialog, onConfirm: () => void confirmProfileExport(), confirmDisabled: profileExportSign && !profileExportPassphrase.trim(), children: [_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Add HMAC metadata") }), _jsx("input", { type: "checkbox", checked: profileExportSign, onChange: (event) => setProfileExportSign(event.target.checked), "aria-label": tr("Profile HMAC metadata") })] }), profileExportSign ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(profileExportTrustState), children: profileExportTrustState }), profileExportKeyHint.trim() ? _jsxs("span", { className: "microcopy", children: [tr("hint"), ": ", profileExportKeyHint.trim()] }) : null] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("HMAC passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: profileExportPassphrase, onChange: (event) => {
                                            setProfileExportPassphrase(event.target.value);
                                            if (profileExportError)
                                                setProfileExportError(null);
                                        }, "aria-label": tr("Profile HMAC passphrase"), placeholder: tr("required when HMAC metadata is enabled") })] }), _jsxs("div", { className: "action-dialog-row", children: [_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Saved key hint") }), _jsxs("select", { className: "action-dialog-select", value: selectedKeyHintProfileId, onChange: (event) => {
                                                ...
                                                }, "aria-label": tr("Saved key hint profile"), children: [_jsx("option", { value: "", children: tr("custom key hint") }), keyHintProfiles.map((profile) => (_jsxs("option", { value: profile.id, children: [profile.name, " \u00B7 ", profile.keyHint] }, profile.id)))] })] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Key hint label") }), _jsx("input", { className: "action-dialog-input", value: profileExportKeyHint, onChange: (event) => setProfileExportKeyHint(event.target.value), "aria-label": tr("Profile key hint"), placeholder: tr("optional recipient-visible hint") })] })] }), _jsxs("div", { className: "action-dialog-row", children: [_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Save key hint profile as") }), _jsx("input", { className: "action-dialog-input", value: keyHintProfileName, onChange: (event) => setKeyHintProfileName(event.target.value), "aria-label": tr("Key hint profile name"), placeholder: tr("for example team-signing-key") })] }), _jsx("button", { type: "button", className: "button", onClick: saveProfileHint, disabled: !profileExportKeyHint.trim(), children: tr("save hint") })] }), _jsxs("p", { className: "action-dialog-note", children: [tr("Key hints are local labels only. Passphrases are not persisted."), selectedKeyHintProfile ? ` Active: ${selectedKeyHintProfile.name} (v${selectedKeyHintProfile.version})` : ""] })] })) : (_jsx("p", { className: "action-dialog-note", children: tr("Unsigned profile exports can still be imported, but HMAC verification is unavailable.") })), profileExportError ? _jsx("p", { className: "action-dialog-error", children: profileExportError }) : null] }), _jsxs(ActionDialog, { open: profileImportOpen, title: tr("Import profile snapshot"), description: profileImportDescriptor
                    ...
                    : tr("Select import settings"), confirmLabel: tr("import profile"), onCancel: closeProfileImportDialog, onConfirm: () => void confirmProfileImport(), children: [profileImportDescriptor?.signed ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(profileImportTrustState), children: profileImportTrustState }), profileImportDescriptor.keyHint ? _jsxs("span", { className: "microcopy", children: [tr("hint"), ": ", profileImportDescriptor.keyHint] }) : null] }), _jsxs("p", { className: "action-dialog-note", children: [tr("HMAC-protected profile detected"), profileImportDescriptor.keyHint ? ` (${tr("hint")}: ${profileImportDescriptor.keyHint})` : "", ". ", tr("NullID uses shared-passphrase HMAC verification before import.")] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Verification passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: profileImportPassphrase, onChange: (event) => {
                                            setProfileImportPassphrase(event.target.value);
                                            if (profileImportError)
                                                setProfileImportError(null);
                                        }, "aria-label": tr("Profile verification passphrase"), placeholder: tr("required for HMAC-protected profile") })] })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(profileImportTrustState), children: profileImportTrustState })] }), _jsx("p", { className: "action-dialog-note", children: tr("Unsigned profile snapshot. Continue only if you trust the source.") })] })), profileImportError ? _jsx("p", { className: "action-dialog-error", children: profileImportError }) : null] }), _jsx(FeedbackWidget, { activeModule: resolvedActiveModule }), _jsx(OnboardingTour, { open: tourOpen, stepIndex: tourStepIndex, steps: onboardingSteps, onStepIndexChange: setTourStepIndex, onSkip: skipOnboarding, onFinish: finishOnboarding })] }));
}
