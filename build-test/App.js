...
function AppShell() {
    ...
    return (_jsxs("div", { className: `app-surface ${isCompact ? "is-compact" : ""}`, children: [_jsx("input", { ref: importProfileInputRef, type: "file", accept: "application/json", style: { position: "absolute", opacity: 0, width: 1, height: 1, pointerEvents: "none" }, tabIndex: -1, onChange: async (event) => {
                    ...
                    : tr("Select import settings"), confirmLabel: tr("import profile"), onCancel: closeProfileImportDialog, onConfirm: () => void confirmProfileImport(), children: [profileImportDescriptor?.signed ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: "trust state" }), _jsx("span", { className: trustTagClass(profileImportTrustState), children: profileImportTrustState }), profileImportDescriptor.keyHint ? _jsxs("span", { className: "microcopy", children: [tr("hint"), ": ", profileImportDescriptor.keyHint] }) : null] }), _jsxs("p", { className: "action-dialog-note", children: [tr("Signed profile detected"), profileImportDescriptor.keyHint ? ` (${tr("hint")}: ${profileImportDescriptor.keyHint})` : "", ". ", tr("Verification is required before import.")] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Verification passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: profileImportPassphrase, onChange: (event) => {
                                            setProfileImportPassphrase(event.target.value);
                                            if (profileImportError)
                                                setProfileImportError(null);
                                        }, "aria-label": tr("Profile verification passphrase"), placeholder: tr("required for signed profile") })] })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: "trust state" }), _jsx("span", { className: trustTagClass(profileImportTrustState), children: profileImportTrustState })] }), _jsx("p", { className: "action-dialog-note", children: tr("Unsigned profile snapshot. Continue only if you trust the source.") })] })), profileImportError ? _jsx("p", { className: "action-dialog-error", children: profileImportError }) : null] }), _jsx(FeedbackWidget, { activeModule: resolvedActiveModule }), _jsx(OnboardingTour, { open: tourOpen, stepIndex: tourStepIndex, steps: onboardingSteps, onStepIndexChange: setTourStepIndex, onSkip: skipOnboarding, onFinish: finishOnboarding })] }));
}
