...
export function VaultView({ onOpenGuide }) {
    ...
    const [selectedKeyHintProfileId, setSelectedKeyHintProfileId] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.selectedKeyHintProfileId, "");
    ...
    const [unlockRateLimitEnabled, setUnlockRateLimitEnabled] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.unlockRateLimitEnabled, true);
    const [unlockHumanCheckEnabled, setUnlockHumanCheckEnabled] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.unlockHumanCheckEnabled, true);
    const [unlockThrottle, setUnlockThrottle] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.unlockThrottle, createUnlockThrottleState());
    ...
    const [sessionCookieEnabled, setSessionCookieEnabled] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.sessionCookieEnabled, true);
    const [sessionCookieState, setSessionCookieState] = useState(null);
    const [mfaCredential, setMfaCredential] = usePersistentState(VAULT_PREFERENCE_STATE_KEYS.mfaCredential, null);
    ...
    const confirmVaultExport = useCallback(async () => {
        ...
        try {
            ...
            push(vaultExportMode === "encrypted"
                ...
                : `vault export ready${vaultExportSign ? " (HMAC metadata)" : ""}`, "accent");
    ...
    return (_jsxs("div", { className: "workspace-scroll", children: [_jsx("div", { className: "guide-link", children: _jsx("button", { type: "button", className: "guide-link-button", onClick: () => onOpenGuide?.("vault"), children: t("guide.link") }) }), _jsxs("div", { className: "grid-two", children: [_jsxs("div", { className: "panel", "aria-label": tr("Vault controls"), children: [_jsxs("div", { className: "panel-heading", children: [_jsx("span", { children: tr("Secure Notes") }), _jsx("span", { className: "panel-subtext", children: tr("AES-GCM + PBKDF2") })] }), _jsxs("div", { className: "controls-row", children: [_jsx("input", { className: "input", type: "password", placeholder: tr("passphrase"), value: passphrase, onChange: (event) => setPassphrase(event.target.value), "aria-label": tr("Vault key") }), _jsx("button", { className: "button", type: "button", onClick: handleUnlock, disabled: unlocked || !passphrase, children: tr("unlock") }), _jsx("button", { className: "button", type: "button", onClick: handleLock, disabled: !unlocked, children: tr("lock") })] }), unlockHumanCheckEnabled && shouldRequireHumanCheck(unlockThrottle, DEFAULT_UNLOCK_POLICY) && !unlocked ? (_jsxs("div", { className: "controls-row", children: [_jsx("span", { className: "section-title", children: tr("Human check") }), _jsx("span", { className: "tag", children: unlockChallenge.prompt }), _jsx("input", { className: "input", value: unlockChallengeInput, onChange: (event) => setUnlockChallengeInput(event.target.value), placeholder: tr("answer required"), "aria-label": tr("Unlock human check") })] })) : null, _jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("unlock hardening") }), _jsx(Chip, { label: unlockRateLimitEnabled ? tr("rate-limit:on") : tr("rate-limit:off"), tone: unlockRateLimitEnabled ? "accent" : "muted" }), _jsx(Chip, { label: unlockHumanCheckEnabled ? tr("human-check:on") : tr("human-check:off"), tone: unlockHumanCheckEnabled ? "accent" : "muted" }), unlockBlockRemaining > 0 ? _jsxs("span", { className: "microcopy", children: [tr("blocked for"), " ", formatNumber(unlockBlockRemaining), " ", tr("sec")] }) : null] }), _jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("passphrase strength") }), _jsx("span", { className: gradeTagClass(passphraseAssessment.grade), children: gradeLabel(passphraseAssessment.grade) }), _jsxs("span", { className: "microcopy", children: [tr("effective"), " \u2248 ", formatNumber(passphraseAssessment.effectiveEntropyBits), " ", tr("bits")] })] }), _jsxs("div", { className: "controls-row", children: [_jsxs("label", { className: "microcopy", style: { display: "flex", alignItems: "center", gap: "0.35rem" }, children: [_jsx("input", { type: "checkbox", checked: unlockRateLimitEnabled, onChange: (event) => {
                                                    setUnlockRateLimitEnabled(event.target.checked);
                                                    if (!event.target.checked) {
                                                        setUnlockThrottle(clearUnlockFailures());
                                                    }
                                                }, "aria-label": tr("Enable unlock rate limit") }), tr("rate limit unlock attempts")] }), _jsxs("label", { className: "microcopy", style: { display: "flex", alignItems: "center", gap: "0.35rem" }, children: [_jsx("input", { type: "checkbox", checked: unlockHumanCheckEnabled, onChange: (event) => {
                                                    setUnlockHumanCheckEnabled(event.target.checked);
                                                    setUnlockChallenge(createHumanCheckChallenge());
                                                    setUnlockChallengeInput("");
                                                }, "aria-label": tr("Enable unlock human check") }), tr("require human check after repeated failures")] }), _jsxs("label", { className: "microcopy", style: { display: "flex", alignItems: "center", gap: "0.35rem" }, children: [_jsx("input", { type: "checkbox", checked: sessionCookieEnabled, onChange: (event) => {
                                                    setSessionCookieEnabled(event.target.checked);
                                                    if (!event.target.checked) {
                                                        clearVaultSessionCookie();
                                                        setSessionCookieState(null);
                                                    }
                                                }, "aria-label": tr("Enable session cookie") }), tr("set session hint cookie on unlock")] })] }), _jsxs("div", { className: "controls-row", children: [_jsx("span", { className: "section-title", children: tr("MFA") }), _jsx(Chip, { label: mfaCredential ? tr("enabled") : tr("disabled"), tone: mfaCredential ? "accent" : "muted" }), _jsx("input", { className: "input", value: mfaLabelInput, onChange: (event) => setMfaLabelInput(event.target.value), placeholder: tr("MFA label (optional)"), "aria-label": tr("MFA label"), disabled: Boolean(mfaCredential) || !unlocked }), _jsx("button", { className: "button", type: "button", onClick: () => void handleEnableMfa(), disabled: Boolean(mfaCredential) || !unlocked || mfaBusy, children: mfaBusy ? tr("working…") : tr("enable mfa") }), _jsx("button", { className: "button", type: "button", onClick: handleDisableMfa, disabled: !mfaCredential, children: tr("disable mfa") })] }), _jsxs("div", { className: "microcopy", children: [mfaCredential
                                        ...
                                        : tr("MFA is optional. Enable it to add a local WebAuthn prompt after passphrase unlock on this browser/device."), !isLocalMfaSupported() ? ` ${tr("WebAuthn is unavailable in this browser/runtime.")}` : "", ` ${tr("Keep a separate vault backup before relying on MFA; losing the authenticator while locked can strand access.")}`] }), _jsxs("div", { className: "controls-row", children: [_jsx("button", { className: "button", type: "button", onClick: () => openVaultExportDialog("plain"), disabled: !unlocked || notes.length === 0, children: tr("export (json)") }), _jsx("button", { className: "button", type: "button", onClick: () => openVaultExportDialog("encrypted"), disabled: !unlocked || notes.length === 0, children: tr("export encrypted") }), _jsx("button", { className: "button", type: "button", onClick: handleImport, children: tr("import") }), _jsx("button", { className: "button", type: "button", onClick: () => encryptedImportRef.current?.click(), children: tr("import encrypted") }), _jsx("input", { ref: fileInputRef, type: "file", accept: "application/json", tabIndex: -1, style: { position: "absolute", opacity: 0, width: 1, height: 1, pointerEvents: "none" }, onChange: async (event) => {
                                        ...
                                        }, disabled: !unlocked || filteredNotes.length === 0, children: tr("export notes report") }), _jsx("span", { className: "microcopy", children: vaultStats.latestUpdate ? `${tr("latest update")}: ${formatTs(vaultStats.latestUpdate)}` : tr("no notes yet") })] }), unlocked ? (filteredNotes.length === 0 ? (_jsx("div", { className: "microcopy", children: tr("no matching notes") })) : (_jsx("ul", { className: "note-list", children: filteredNotes.map((note) => (_jsxs("li", { children: [_jsx("div", { className: "note-title", children: note.title }), _jsx("div", { className: "note-body", children: note.body }), note.tags.length > 0 && (_jsx("div", { className: "controls-row", children: note.tags.map((tag) => (_jsx(Chip, { label: tag, tone: "muted" }, tag))) })), _jsxs("div", { className: "microcopy", children: [tr("created"), " ", formatTs(note.createdAt), " \u00B7 ", tr("updated"), " ", formatTs(note.updatedAt)] }), _jsxs("div", { className: "controls-row", children: [_jsx("button", { className: "button", type: "button", onClick: () => handleEdit(note), children: tr("edit") }), _jsx("button", { className: "button", type: "button", onClick: () => handleDelete(note.id), style: { borderColor: "var(--danger)", color: "var(--danger)" }, children: tr("delete") })] })] }, note.id))) }))) : (_jsx("div", { className: "microcopy", children: tr("locked. unlock to view.") }))] })] }), _jsxs(ActionDialog, { open: vaultExportDialogOpen, title: vaultExportMode === "encrypted" ? tr("Export encrypted vault snapshot") : tr("Export vault snapshot"), description: tr("NullID uses shared-passphrase HMAC metadata for export verification. It is not a public-key identity signature."), confirmLabel: vaultExportMode === "encrypted" ? tr("export encrypted") : tr("export snapshot"), onCancel: closeVaultExportDialog, onConfirm: () => void confirmVaultExport(), confirmDisabled: (vaultExportMode === "encrypted" && !vaultExportPassphrase.trim()) || (vaultExportSign && !vaultSigningPassphrase.trim()), children: [vaultExportMode === "encrypted" ? (_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Export passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: vaultExportPassphrase, onChange: (event) => {
                                    setVaultExportPassphrase(event.target.value);
                                    if (vaultExportError)
                                        setVaultExportError(null);
                                }, "aria-label": tr("Vault export passphrase"), placeholder: tr("required for encrypted export") })] })) : null, _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Add HMAC metadata") }), _jsx("input", { type: "checkbox", checked: vaultExportSign, onChange: (event) => setVaultExportSign(event.target.checked), "aria-label": tr("Vault export HMAC metadata") })] }), vaultExportSign ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(vaultExportTrustState), children: vaultExportTrustState }), vaultExportKeyHint.trim() ? _jsxs("span", { className: "microcopy", children: [tr("hint"), ": ", vaultExportKeyHint.trim()] }) : null] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("HMAC passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: vaultSigningPassphrase, onChange: (event) => {
                                            setVaultSigningPassphrase(event.target.value);
                                            if (vaultExportError)
                                                setVaultExportError(null);
                                        }, "aria-label": tr("Vault HMAC passphrase"), placeholder: tr("required when HMAC metadata is enabled") })] }), _jsxs("div", { className: "action-dialog-row", children: [_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Saved key hint") }), _jsxs("select", { className: "action-dialog-select", value: selectedKeyHintProfileId, onChange: (event) => {
                                                ...
                                                }, "aria-label": tr("Saved vault key hint profile"), children: [_jsx("option", { value: "", children: tr("custom key hint") }), keyHintProfiles.map((profile) => (_jsxs("option", { value: profile.id, children: [profile.name, " \u00B7 ", profile.keyHint] }, profile.id)))] })] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Key hint label") }), _jsx("input", { className: "action-dialog-input", value: vaultExportKeyHint, onChange: (event) => setVaultExportKeyHint(event.target.value), "aria-label": tr("Vault key hint"), placeholder: tr("optional verification hint") })] })] }), _jsxs("p", { className: "action-dialog-note", children: [tr("Key hints are local labels only; passphrases are never persisted."), selectedKeyHintProfile ? ` Active: ${selectedKeyHintProfile.name} (v${selectedKeyHintProfile.version})` : ""] })] })) : (_jsx("p", { className: "action-dialog-note", children: tr("Unsigned exports skip HMAC verification during import.") })), vaultExportError ? _jsx("p", { className: "action-dialog-error", children: vaultExportError }) : null] }), _jsxs(ActionDialog, { open: vaultImportDialogOpen, title: vaultImportMode === "encrypted" ? tr("Import encrypted vault snapshot") : tr("Import vault snapshot"), description: vaultImportMode === "encrypted"
                    ? tr("Provide export passphrase and optional verification passphrase.")
                    : `${vaultImportDescriptor?.noteCount ?? 0} notes · schema ${vaultImportDescriptor?.schemaVersion ?? "unknown"}`, confirmLabel: vaultImportMode === "encrypted" ? tr("import encrypted") : tr("import snapshot"), onCancel: closeVaultImportDialog, onConfirm: () => void confirmVaultImport(), children: [vaultImportMode === "encrypted" ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(vaultImportTrustState), children: vaultImportTrustState })] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Export passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: vaultImportExportPassphrase, onChange: (event) => {
                                            setVaultImportExportPassphrase(event.target.value);
                                            if (vaultImportError)
                                                setVaultImportError(null);
                                        }, "aria-label": tr("Encrypted vault import passphrase"), placeholder: tr("required") })] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Verification passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: vaultImportVerifyPassphrase, onChange: (event) => {
                                            setVaultImportVerifyPassphrase(event.target.value);
                                            if (vaultImportError)
                                                setVaultImportError(null);
                                        }, "aria-label": tr("Encrypted vault verification passphrase"), placeholder: tr("required when snapshot metadata is signed") })] })] })) : vaultImportDescriptor?.signed ? (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(vaultImportTrustState), children: vaultImportTrustState }), vaultImportDescriptor.keyHint ? _jsxs("span", { className: "microcopy", children: [tr("hint"), ": ", vaultImportDescriptor.keyHint] }) : null] }), _jsxs("p", { className: "action-dialog-note", children: [tr("HMAC-protected snapshot detected"), vaultImportDescriptor.keyHint ? ` (${tr("hint")}: ${vaultImportDescriptor.keyHint})` : "", ". ", tr("NullID uses shared-passphrase HMAC verification before import."), " ", tr("Verification is required before import.")] }), _jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Verification passphrase") }), _jsx("input", { className: "action-dialog-input", type: "password", value: vaultImportVerifyPassphrase, onChange: (event) => {
                                            setVaultImportVerifyPassphrase(event.target.value);
                                            if (vaultImportError)
                                                setVaultImportError(null);
                                        }, "aria-label": tr("Vault verification passphrase"), placeholder: tr("required for HMAC-verified snapshots") })] })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { className: "status-line", children: [_jsx("span", { children: tr("trust state") }), _jsx("span", { className: trustTagClass(vaultImportTrustState), children: vaultImportTrustState })] }), _jsx("p", { className: "action-dialog-note", children: tr("Unsigned snapshot. Continue only if you trust this file.") })] })), vaultImportError ? _jsx("p", { className: "action-dialog-error", children: vaultImportError }) : null] }), _jsxs(ActionDialog, { open: reportDialogOpen, title: tr("Export notes report"), description: tr("Exports plain JSON. Include note bodies only if you are comfortable with plaintext note content in the report."), confirmLabel: tr("export report"), onCancel: () => setReportDialogOpen(false), onConfirm: () => {
                    exportFilteredReport(reportIncludeBodies);
                    setReportDialogOpen(false);
                }, children: [_jsxs("label", { className: "action-dialog-field", children: [_jsx("span", { children: tr("Include note bodies in report") }), _jsx("input", { type: "checkbox", checked: reportIncludeBodies, onChange: (event) => setReportIncludeBodies(event.target.checked), "aria-label": tr("Include note bodies") })] }), _jsx("p", { className: "action-dialog-note", children: tr("If enabled, exported note bodies are written in plaintext JSON.") })] }), _jsx(ActionDialog, { open: wipeDialogOpen, title: tr("Wipe vault data"), description: tr("This removes all vault metadata, canary records, and encrypted notes from local storage."), confirmLabel: tr("wipe vault"), danger: true, onCancel: () => setWipeDialogOpen(false), onConfirm: () => {
                    void handleWipe();
