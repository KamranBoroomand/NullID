...
export function PwView({ onOpenGuide }) {
    ...
    const localizedGradeLabel = (grade) => tr(gradeLabel(grade));
    const toggleKeyLabel = {
        upper: tr("upper"),
        lower: tr("lower"),
        digits: tr("digits"),
        symbols: tr("symbols"),
...
function formatGuessRate(rate) {
    if (rate >= 1_000_000_000)
        return `${(rate / 1_000_000_000).toFixed(1)}B/s`;
    if (rate >= 1_000_000)
