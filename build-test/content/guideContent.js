export const guideTools = [
    ...
    },
    {
        key: "sanitize",
        title: "Log Sanitizer",
        whatItDoes: "Rule-based scrubbing for logs with diff preview. Rules cover IP + IPv6, emails, JWT/bearer tokens, AWS keys/secrets, IBAN/credit cards, cookies, UAs, timestamps, usernames, and JSON secrets.",
    ...
    },
    {
        ...
        whatItDoes: "Reads EXIF from images and re-encodes them to strip metadata, with optional resize and before/after previews.",
        whatWhen: ["Clear camera/location data before sharing images", "Downsize images while removing EXIF"],
        howSteps: ["Drop an image (JPEG/PNG/WebP); metadata is parsed locally.", "Choose resize before exporting.", "Download the cleaned image and review before/after previews."],
        limits: [
            "HEIC is flagged as unsupported; do not assume it is stripped.",
            "Canvas re-encode may slightly change colors/compression.",
    ...
    },
    {
        ...
        howSteps: [
            ...
            "Export/import (plain or encrypted) for offline backup; wipe clears IndexedDB.",
        ],
        limits: [
...
export const guideExtras = [
    {
        ...
        whatItDoes: "Save or load preferences stored under the nullid:* keys in localStorage (not the vault DB).",
        ...
        howSteps: [
            ...
            "Import via System → Import profile and choose the JSON file; the app reloads to apply.",
            "Schema versioning guards incompatible snapshots; only keys under nullid:* are written back.",
        ],
        limits: [
            "Vault content lives in IndexedDB and is not included; export the vault separately.",
            "Only well-typed values are applied; malformed JSON is rejected.",
