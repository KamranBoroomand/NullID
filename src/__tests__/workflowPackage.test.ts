...
import {
  createSanitizeSafeShareBundle,
  createWorkflowPackage,
  describeWorkflowPackagePayload,
  extractWorkflowPackage,
  normalizeWorkflowPackage,
  SAFE_SHARE_BUNDLE_KIND,
  SAFE_SHARE_BUNDLE_SCHEMA_VERSION,
  verifyWorkflowPackagePayload,
  WORKFLOW_PACKAGE_KIND,
  WORKFLOW_PACKAGE_SCHEMA_VERSION,
} from "../utils/workflowPackage.js";
...
describe("workflow package contract", () => {
  ...
  it("normalizes unsupported inner signature and encryption claims to honest none-based trust metadata", async () => {
    ...
    assert.equal(verified.trustBasis.some((line) => /shared-secret-hmac|NULLID:ENC:2/i.test(line)), false);
  });
