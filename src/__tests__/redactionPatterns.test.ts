...
describe("redaction patterns", () => {
  ...
  it("keeps browser redaction report exports free of exact original findings", () => {
    const source = fs.readFileSync("src/views/RedactView.tsx", "utf8");

    assert.doesNotMatch(source, /original:\s*change\.original/u);
    assert.doesNotMatch(source, /fingerprintText|fingerprint:/u);
    assert.match(source, /redactedPreview|originalLength/u);
  });

  it("guards RedactView custom regex additions with the shared worker validator", () => {
    const source = fs.readFileSync("src/views/RedactView.tsx", "utf8");

    assert.match(source, /validateCustomRegexRule/u);
    assert.doesNotMatch(source, /new RegExp\(customPattern,\s*"gi"\)/u);
    assert.doesNotMatch(source, /new RegExp\(pattern,\s*"gi"\)/u);
  });

  it("applies the minimum severity threshold to custom redaction findings", async () => {
    const text = "internal marker: project-raven";
    const custom = [{ label: "project marker", pattern: "project-raven", flags: "g" }];

    const high = await import("../utils/redaction.js").then(({ scanRedactionAsync }) =>
      scanRedactionAsync(text, [], custom, { minimumSeverity: "high", minTokenLength: 20 }),
    );
    const medium = await import("../utils/redaction.js").then(({ scanRedactionAsync }) =>
      scanRedactionAsync(text, [], custom, { minimumSeverity: "medium", minTokenLength: 20 }),
    );
    const low = await import("../utils/redaction.js").then(({ scanRedactionAsync }) =>
      scanRedactionAsync(text, [], custom, { minimumSeverity: "low", minTokenLength: 20 }),
    );

    assert.equal(high.total, 0);
    assert.equal(medium.total, 1);
    assert.equal(medium.matches[0]?.severity, "medium");
    assert.equal(low.total, 1);
    assert.equal(low.matches[0]?.severity, "medium");
  });

  it("keeps mixed built-in and custom findings deterministic across thresholds", async () => {
    const text = "email alice@example.com project-raven";
    const custom = [{ label: "project marker", pattern: "project-raven", flags: "g" }];
    const detectors = getRedactionDetectors().filter((detector) => detector.key === "email");

    const medium = await import("../utils/redaction.js").then(({ scanRedactionAsync }) =>
      scanRedactionAsync(text, detectors, custom, { minimumSeverity: "medium", minTokenLength: 20 }),
    );
    const high = await import("../utils/redaction.js").then(({ scanRedactionAsync }) =>
      scanRedactionAsync(text, detectors, custom, { minimumSeverity: "high", minTokenLength: 20 }),
    );

    assert.deepEqual(
      medium.matches.map((match) => `${match.start}:${match.label}:${match.severity}`),
      ["6:Email:medium", "24:project marker:medium"],
    );
    assert.equal(high.total, 0);
  });
});
