...
import { applySanitizeRulesAsync, buildRulesState, normalizePolicyConfig, type CustomRule } from "../utils/sanitizeEngine.js";
...
describe("adversarial corpus", () => {
  ...
  it("keeps hostile nested-quantifier regex rules worker-only after normalization", () => {
    ...
    assert.equal(normalized?.customRules.length, 1);
  });

  it("skips hostile regex payloads at runtime when injected directly", async () => {
    const dangerous: CustomRule = {
      id: "danger",
      pattern: "(a+)+$",
      replacement: "[x]",
      flags: "g",
    ...
    const result = await applySanitizeRulesAsync(input, buildRulesState([]), [dangerous], false);
    assert.equal(result.output, input);
    assert.equal(result.report.some((entry) => /failed \((timeout|worker-error|budget|syntax-error)\)/i.test(entry)), true);
  });
  ...
  it("rejects tampered envelope ciphertext", async () => {
    ...
    const parsed = JSON.parse(bytesToUtf8(fromBase64Url(encoded))) as { header: unknown; ciphertext: string };
