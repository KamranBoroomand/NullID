...
function encodeBase64(byteLength: number, fill: number): string {
  return Buffer.alloc(byteLength, fill).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/u, "");
}

function encodeStandardBase64(byteLength: number, fill: number): string {
...
describe("password hashing", () => {
  ...
  it("rejects imported records with padded standard base64 fields", () => {
    const message = getErrorMessage(() =>
      parsePasswordHash(
        `$sha256$s=${encodeStandardBase64(PASSWORD_HASH_LIMITS.saltBytes.min, 0xff)}$${encodeStandardBase64(PASSWORD_HASH_LIMITS.record.derivedBytes, 0x11)}`,
      ),
    );
