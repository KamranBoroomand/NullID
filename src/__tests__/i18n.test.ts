...
describe("i18n locale contract", () => {
  ...
  it("keeps non-English message keys aligned with English", () => {
    ...
    for (const locale of ["fa", "ru"] as const) {
  ...
  it("keeps critical runtime phrases localized without obvious English leakage", () => {
    const checks: Array<{ phrase: string; faExcludes?: RegExp; ruExcludes?: RegExp; faIncludes?: RegExp; ruIncludes?: RegExp }> = [
    ...
    for (const check of checks) {
      ...
      if (!entry) continue;
      if (check.faExcludes) assert.equal(check.faExcludes.test(entry.fa), false);
      if (check.ruExcludes) assert.equal(check.ruExcludes.test(entry.ru), false);
      if (check.faIncludes) assert.equal(check.faIncludes.test(entry.fa), true);
      if (check.ruIncludes) assert.equal(check.ruIncludes.test(entry.ru), true);
    }
  });
