...
describe("safe share assistant", () => {
  it("creates a text workflow package with preset metadata and honest trust labels", async () => {
    ...
    assert.match(workflowPackage.limitations.join(" "), /NULLID:ENC:2 protection is applied to the exported file/i);
  ...
  it("blocks text workflow package export when custom regex rules fail", async () => {
    await assert.rejects(
      createSafeShareTextWorkflowPackage({
        ...
        policyOverride: {
          ...
          jsonAware: false,
          customRules: [{ id: "broken-custom", pattern: "(", flags: "g", replacement: "[custom]", scope: "text" }],
        },
      }),
      /failed \(syntax-error\)|Custom regex/u,
    );
  });
