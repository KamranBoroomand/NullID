...
import { normalizePolicyConfig, type PolicyPack, type SanitizePolicyConfig } from "./sanitizeEngine.js";
...
type WorkflowPackageSurface = "web" | "cli" | "unknown";
type WorkflowPackageType = "bundle" | "report";
type WorkflowPackageTrustIdentity = "not-asserted";
type WorkflowPackageSignatureMethod = "none";
type WorkflowPackageEncryptionMethod = "none";
type WorkflowPackageArtifactKind = "text" | "json" | "binary" | "manifest" | "reference";

interface WorkflowPackagePreset {
  id: string;
  label: string;
  summary?: string;
}

export interface WorkflowPackageProducer {
  app: "NullID";
  surface: WorkflowPackageSurface;
  module?: string;
  version?: string | null;
  buildId?: string | null;
}

interface WorkflowPackageSummary {
  title: string;
  description: string;
  highlights: string[];
}

interface WorkflowPackageReport {
  purpose?: string;
  audience?: string;
...
interface WorkflowPackageTrust {
  identity: WorkflowPackageTrustIdentity;
  packageSignature: {
    method: WorkflowPackageSignatureMethod;
    keyHint?: string;
  };
  artifactManifest: {
    algorithm: "sha256";
  ...
  encryptedPayload: {
    method: WorkflowPackageEncryptionMethod;
  };
  notes: string[];
}

export interface WorkflowPackageArtifact {
  id: string;
  role: string;
  label: string;
  kind: WorkflowPackageArtifactKind;
  mediaType: string;
  included: boolean;
  bytes?: number;
  sha256?: string;
...
export interface WorkflowPackageTransform {
  id: string;
  type: string;
  label: string;
  summary: string;
...
export interface WorkflowPackage {
  ...
  kind: typeof WORKFLOW_PACKAGE_KIND;
  packageType: WorkflowPackageType;
  workflowType: string;
  producedAt: string;
  producer: WorkflowPackageProducer;
  workflowPreset?: WorkflowPackagePreset;
  summary: WorkflowPackageSummary;
  report?: WorkflowPackageReport;
  trust: WorkflowPackageTrust;
  artifacts: WorkflowPackageArtifact[];
  policy?: {
    type: "sanitize";
    config?: SanitizePolicyConfig;
    preset?: string;
    packName?: string;
    baseline?: string | null;
  };
  transforms?: WorkflowPackageTransform[];
  warnings: string[];
  limitations: string[];
}

export interface WorkflowPackageDescriptor {
  schemaVersion: number;
  kind: string;
  packageType: string;
  workflowType: string;
  artifactCount: number;
  signatureMethod: WorkflowPackageSignatureMethod | "unknown";
  encryptionMethod: WorkflowPackageEncryptionMethod | "unknown";
  legacy: boolean;
  sourceKind: "workflow-package" | "safe-share";
}

type WorkflowPackageVerificationState = "unsigned" | "integrity-checked" | "mismatch" | "invalid";

interface WorkflowPackageArtifactVerification {
  id: string;
  role: string;
  label: string;
  kind: WorkflowPackageArtifactKind;
...
export interface WorkflowPackageVerificationResult {
  descriptor: WorkflowPackageDescriptor;
  workflowPackage: WorkflowPackage;
  verificationState: WorkflowPackageVerificationState;
  ...
  warnings: string[];
  limitations: string[];
  artifactChecks: WorkflowPackageArtifactVerification[];
  failure?: string;
}

export interface SafeShareBundle {
  schemaVersion: number;
  kind: typeof SAFE_SHARE_BUNDLE_KIND;
  createdAt: string;
  tool: "sanitize";
  producer: WorkflowPackageProducer;
  sourceFile?: string;
  detectedFormat?: string;
  policy: SanitizePolicyConfig;
  input: {
    bytes: number;
    sha256: string;
  };
...
interface WorkflowPackageInput {
  packageType: WorkflowPackageType;
  workflowType: string;
  producedAt: string;
  producer: WorkflowPackageProducer;
  ...
  summary: WorkflowPackageSummary;
  report?: WorkflowPackageReport;
  trust?: Partial<WorkflowPackageTrust>;
  artifacts: WorkflowPackageArtifact[];
  policy?: WorkflowPackage["policy"];
  transforms?: WorkflowPackageTransform[];
  warnings?: string[];
  limitations?: string[];
}

export interface CreateSanitizeWorkflowPackageInput {
  producedAt?: string;
  producer: WorkflowPackageProducer;
  sourceFile?: string;
  detectedFormat?: string;
...
export function createWorkflowPackage(input: WorkflowPackageInput): WorkflowPackage {
...
function createSanitizeWorkflowPackage(input: CreateSanitizeWorkflowPackageInput): WorkflowPackage {
...
export function createSanitizeSafeShareBundle(input: CreateSanitizeWorkflowPackageInput): SafeShareBundle {
...
export function describeWorkflowPackagePayload(input: unknown): WorkflowPackageDescriptor {
...
export function extractWorkflowPackage(input: unknown): WorkflowPackage {
  const direct = normalizeWorkflowPackage(input);
...
export function normalizeWorkflowPackage(input: unknown): WorkflowPackage | null {
  if (!isRecord(input)) return null;
  if (input.kind !== WORKFLOW_PACKAGE_KIND) return null;
  if (Number(input.schemaVersion) !== WORKFLOW_PACKAGE_SCHEMA_VERSION) return null;
...
export async function verifyWorkflowPackagePayload(input: unknown): Promise<WorkflowPackageVerificationResult> {
  ...
  let verificationState: WorkflowPackageVerificationState = "unsigned";
...
function extractLegacySafeSharePackage(input: Record<string, unknown>): WorkflowPackage | null {
  ...
  if (!inputMeta || !outputMeta || typeof outputMeta.text !== "string") return null;
  ...
  return createWorkflowPackage({
    ...
    warnings: normalizeUniqueStringList(
      normalizeStringList(input.warnings, 40, 240).concat(["Unsigned package. Sender identity is not asserted."]),
      40,
      240,
    ),
    limitations: normalizeUniqueStringList(input.limitations, 40, 240),
...
function normalizePackageType(value: unknown): WorkflowPackageType {
  return value === "report" ? "report" : "bundle";
}

function describeWorkflowPackageFailure(input: unknown) {
  if (!isRecord(input)) {
...
function normalizeProducer(value: unknown): WorkflowPackageProducer {
...
function normalizeSurface(value: unknown): WorkflowPackageSurface {
  if (value === "web" || value === "cli") return value;
  return "unknown";
}

function normalizeSummary(value: unknown): WorkflowPackageSummary {
  const record = isRecord(value) ? value : {};
  return {
...
function normalizeReport(value: unknown): WorkflowPackageReport | undefined {
...
function normalizeTrust(value: unknown, artifactCount: number): WorkflowPackageTrust {
  const record = isRecord(value) ? value : {};
  ...
  return {
    ...
    artifactManifest: {
      algorithm: "sha256",
      entryCount:
        typeof manifestRecord.entryCount === "number" && Number.isInteger(manifestRecord.entryCount) && manifestRecord.entryCount >= 0
    ...
    notes: normalizeUniqueStringList(
      normalizeStringList(record.notes ?? defaultTrustNotes(), 12, 240).concat(
        ignoredPackageSignatureClaim
        ...
        ignoredEncryptedPayloadClaim
          ? ["Ignored unsupported inner workflow package encryption claim. NULLID:ENC:2 applies only to the optional outer export envelope."]
          : [],
      ),
      12,
      240,
...
function normalizeSignatureMethod(_value: unknown): WorkflowPackageSignatureMethod {
  return "none";
}

function normalizeEncryptionMethod(_value: unknown): WorkflowPackageEncryptionMethod {
  return "none";
}

function hasUnsupportedWorkflowTrustClaim(value: unknown) {
  return typeof value === "string" && value !== "none";
}

function normalizeArtifact(value: unknown, index: number): WorkflowPackageArtifact {
  const record = isRecord(value) ? value : {};
  const binaryBase64 = typeof record.base64 === "string" ? sanitizeBase64Payload(record.base64, index) : undefined;
  const artifact: WorkflowPackageArtifact = {
...
function normalizeArtifactKind(value: unknown): WorkflowPackageArtifactKind {
...
async function verifyWorkflowArtifact(artifact: WorkflowPackageArtifact): Promise<WorkflowPackageArtifactVerification> {
  const base: WorkflowPackageArtifactVerification = {
    id: artifact.id,
    role: artifact.role,
...
function normalizeTransform(value: unknown, index: number): WorkflowPackageTransform {
  const record = isRecord(value) ? value : {};
  return {
...
function normalizePolicy(value: unknown): WorkflowPackage["policy"] | undefined {
  if (!isRecord(value)) return undefined;
  const type = value.type === "sanitize" ? "sanitize" : null;
  if (!type) return undefined;

  const next: WorkflowPackage["policy"] = {
    type,
  };
  ...
  if (packName) next.packName = packName;
...
function normalizeWorkflowPreset(value: unknown): WorkflowPackagePreset | undefined {
  if (!isRecord(value)) return undefined;
...
function normalizeSha256(value: unknown, errorMessage: string) {
...
function sanitizeBase64Payload(value: string, index: number) {
...
function normalizeByteSize(value: unknown, errorMessage: string) {
...
function normalizeIso(value: unknown, errorMessage: string) {
...
function normalizeUniqueStringList(value: unknown, maxItems: number, maxLength: number) {
  return Array.from(new Set(normalizeStringList(value, maxItems, maxLength))).slice(0, maxItems);
}

function sanitizeInlineString(value: unknown, maxLength: number, label: string) {
...
function sanitizeOptionalInlineString(value: unknown, maxLength: number) {
