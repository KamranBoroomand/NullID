...
import { canonicalSemanticIdentity } from "./semanticIdentity.js";
...
const POLICY_PACK_SCHEMA_VERSION = 2;
...
type PolicyPackVerificationState = "unsigned" | "integrity-checked" | "verified" | "verification-required" | "mismatch" | "invalid";

export interface PolicyPackVerificationResult extends PolicyPackDescriptor {
  verificationState: PolicyPackVerificationState;
...
export async function verifyPolicyPackPayload(input: unknown, options?: PolicyPackImportOptions): Promise<PolicyPackVerificationResult> {
...
export function mergePolicyPacks(existing: PolicyPack[], incoming: PolicyPack[]): PolicyPack[] {
  const byName = new Map(existing.map((pack) => [canonicalSemanticIdentity(pack.name), pack]));
  incoming.forEach((pack) => {
    byName.set(canonicalSemanticIdentity(pack.name), pack);
  });
...
function invalidPolicyPackResult(descriptor: PolicyPackDescriptor, failure: string): PolicyPackVerificationResult {
  return {
    ...descriptor,
...
function policyPackErrorResult(
  descriptor: PolicyPackDescriptor,
  error: unknown,
  packNames: string[],
  exportedAt?: string,
): PolicyPackVerificationResult {
