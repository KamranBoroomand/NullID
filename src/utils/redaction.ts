...
import { findCustomRegexMatches, type RegexRuleInput } from "./customRegex.js";
...
export function scanRedaction(
  ...
  if (custom.length > 0) {
    const pending = summarizeRedactionMatches(matches);
    return {
      ...pending,
      customStatus: "pending",
      customDiagnostics: ["Custom regex redaction rules are still pending in the worker path"],
      commitAllowed: false,
    };
  }

  return {
    ...summarizeRedactionMatches(matches),
    customStatus: "idle",
    customDiagnostics: [],
    commitAllowed: true,
  };
...
export async function scanRedactionAsync(
  text: string,
  rules: RedactionDetector[],
  custom: RedactionCustomRule[],
  options: ScanRedactionOptions,
  ...
  const base = scanRedaction(text, rules, [], options);
  if (custom.length === 0) return base;
  const minimumRank = severityRank[options.minimumSeverity];
  const customSeverity: RedactionSeverity = "medium";
  const customRules: RegexRuleInput[] = custom.map((rule) => ({
    id: rule.label,
    label: rule.label,
  ...
  const customResults = await findCustomRegexMatches(text, customRules, runOptions);
  const firstError = customResults.find((result) => result.error)?.error;
  if (firstError) {
    const status = firstError.code === "cancelled" ? "cancelled" : "failed";
    return {
      ...base,
      customStatus: status,
      customDiagnostics: customResults
        .filter((result) => result.error)
        .map((result) => `Custom ${result.rule.label ?? result.rule.id}: failed (${result.error!.code}) ${result.error!.message}`),
      commitAllowed: false,
    };
  }
  const customMatches = customResults.flatMap((result): RedactionMatch[] =>
    severityRank[customSeverity] < minimumRank
      ? []
      : result.matches.map((match) => ({
          start: match.start,
          end: match.end,
          key: result.rule.label ?? result.rule.id,
          label: result.rule.label ?? result.rule.id,
          severity: customSeverity,
          mask: `[${result.rule.label ?? result.rule.id}]`,
          ruleSet: "general",
          detectionKind: "heuristic",
        })),
  );
  return {
    ...summarizeRedactionMatches([...base.matches, ...customMatches]),
...
function summarizeRedactionMatches(matches: RedactionMatch[]): RedactionScanResult {
...
function preserveMask(value: string, mask: string) {
...
function normalizeIranPostalCode(value: string) {
  const digits = normalizeMixedIdentifier(value).replace(/[^0-9]/g, "");
...
function normalizeIranSheba(value: string) {
...
function hasKnownIranBankCardPrefix(value: string) {
  const digits = normalizeMixedIdentifier(value).replace(/[^0-9]/g, "");
...
function isValidIranBankCardContext(value: string) {
...
function isValidIranPostalCode(value: string) {
  const digits = normalizeIranPostalCode(value) ?? "";
...
function isValidIranPostalCodeContext(value: string) {
...
function isValidRussianInn(value: string) {
  const digits = normalizeMixedIdentifier(value).replace(/[^0-9]/g, "");
...
function isValidRussianSnils(value: string) {
  const digits = normalizeMixedIdentifier(value).replace(/[^0-9]/g, "");
...
function isValidRussianPassport(value: string) {
  const digits = normalizeMixedIdentifier(value).replace(/[^0-9]/g, "");
...
function isValidRussianPassportContext(value: string) {
...
function isLikelyRussianDocumentContext(value: string) {
