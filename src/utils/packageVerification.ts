import { decryptText, inspectEnvelope, ENVELOPE_PREFIX, LEGACY_ENVELOPE_PREFIX, type EnvelopeInspectResult } from "./cryptoEnvelope.js";
...
interface VerificationArtifactEntry {
  id: string;
  role: string;
  label: string;
  detail: string;
...
interface VerificationWorkflowReport {
  purpose?: string;
  audience?: string;
...
interface VerificationTransformSummary {
  label: string;
  summary: string;
  applied: string[];
  report: string[];
...
interface VerificationReceiverExplanation {
  verified: string[];
  declaredOnly: string[];
  notProvable: string[];
  manualReview: string[];
...
interface VerificationDescriptiveWorkflowMetadata {
  title?: string;
...
export async function inspectReceivedArtifact(
  ...
  if (!trimmed) {
    return finalizeResult({
      ...
      trustBasis: [],
      verifiedChecks: [],
      unverifiedChecks: [`Paste a JSON payload or ${ENVELOPE_PREFIX} envelope, or load a local file first.`],
  ...
  if (trimmed.startsWith(`${ENVELOPE_PREFIX}.`) || trimmed.startsWith(`${LEGACY_ENVELOPE_PREFIX}.`)) {
  ...
  try {
    ...
    return finalizeResult({
      ...
      trustBasis: [],
      verifiedChecks: [],
      unverifiedChecks: [`The content is not valid JSON and is not a ${ENVELOPE_PREFIX} or legacy ${LEGACY_ENVELOPE_PREFIX} envelope.`],
...
async function inspectParsedOrMalformedAfterDecrypt(
  ...
  try {
    ...
    return finalizeResult({
      ...
      envelope,
      trustBasis: [
        `${ENVELOPE_PREFIX} envelope decrypted locally.`,
        ...inner.trustBasis,
      ],
    });
  } catch {
...
function envelopeFactsFromMeta(meta: EnvelopeInspectResult): VerificationFact[] {
  return [
    { label: "Envelope", value: meta.header.version === 2 ? ENVELOPE_PREFIX : LEGACY_ENVELOPE_PREFIX },
    { label: "Metadata authenticated", value: meta.metadataAuthenticated ? "yes" : "no" },
