...
const safeSharePresets: Record<SafeSharePresetId, SafeSharePreset> = {
  "general-safe-share": {
...
function classifyMetadataAnalysisForSafeShare(analysis: MetadataAnalysisResult): SafeShareShareClass {
...
export async function createSafeShareTextWorkflowPackage(input: CreateSafeShareTextPackageInput): Promise<WorkflowPackage> {
  ...
  const sanitize = await applySanitizeRulesAsync(input.inputText, policy.rulesState, policy.customRules, policy.jsonAware);
  if (!sanitize.commitAllowed) {
    throw new Error(sanitize.customDiagnostics[0] ?? "Custom regex rules did not complete; package export blocked");
  }
  ...
  return createWorkflowPackage({
    ...
    report: {
      ...
      receiverCanVerify: [
        ...
        ...(input.protectAtExport ? ["If wrapped at export, the outer NULLID:ENC:2 envelope can be decrypted and integrity-checked with the passphrase."] : []),
      ],
