...
type SupportState = "yes" | "no" | "unknown" | "n/a";
...
const outputMimes: OutputMime[] = ["image/png", "image/jpeg", "image/webp"];
...
const formatRows: { key: string; label: string; mime: string; note?: string }[] = [
  ...
  { key: "avif", label: "AVIF", mime: "image/avif", note: "inspection only; browser cleaning is blocked until dimensions can be proven safely" },
...
export async function probeImageFormatDiagnostics(): Promise<ImageFormatDiagnostics> {
  ...
  const rows = await Promise.all(
    formatRows.map(async (format) => {
      ...
      const cleanExport: SupportState = format.key === "avif" || format.key === "tiff" ? "no" : decode === "no" ? "no" : fallbackClean ? "yes" : "no";
      return {
        ...format,
        decode,
