type PathPrivacyFindingCategory = "names" | "employee-ids" | "case-ids" | "project-names" | "usernames-hostnames";
type PathPrivacyConfidence = "low" | "medium" | "high";

interface PathSegmentFlag {
  segment: string;
  index: number;
  start: number;
  end: number;
...
interface PathPrivacySuggestion {
  original: string;
  preview: string;
  replacements: Array<{
    segment: string;
