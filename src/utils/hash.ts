...
async function finalizeHash(hasher: Hasher, onProgress?: (percent: number) => void): Promise<HashResult> {
...
export async function hashText(text: string, algorithm: HashAlgorithm, options?: HashOptions): Promise<HashResult> {
  ...
  if (encoded.byteLength === 0) {
    return finalizeHash(hasher, options?.onProgress);
  }
  for (let i = 0; i < encoded.byteLength; i += HASH_TEXT_CHUNK) {
    ensureActive(options?.signal);
    const slice = encoded.subarray(i, Math.min(i + HASH_TEXT_CHUNK, encoded.byteLength));
    hasher.update(slice);
    reportProgress(((i + slice.byteLength) / encoded.byteLength) * 100);
    lastYield = await yieldToMain(lastYield);
  }
  return finalizeHash(hasher, options?.onProgress);
}

export async function hashBytes(bytes: Uint8Array, algorithm: HashAlgorithm, options?: HashOptions): Promise<HashResult> {
