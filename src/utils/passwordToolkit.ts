...
export type PassphraseDictionaryProfile = keyof typeof dictionarySegments;
type PassphraseSeparator = "space" | "-" | "." | "_" | "/" | ":";
type PassphraseCaseStyle = "lower" | "title" | "random" | "upper";
type PassphraseNumberMode = "none" | "append-2" | "append-4";
type PassphraseSymbolMode = "none" | "append" | "wrap";

export type PassphraseSettings = {
  words: number;
  separator: PassphraseSeparator;
  dictionaryProfile: PassphraseDictionaryProfile;
  caseStyle: PassphraseCaseStyle;
  numberMode: PassphraseNumberMode;
  symbolMode: PassphraseSymbolMode;
  ensureUniqueWords: boolean;
};

export type SecretGrade = "critical" | "weak" | "fair" | "strong" | "elite";
type CrackScenarioKey = "online-rate-limited" | "online-unthrottled" | "offline-slow-kdf" | "offline-fast-hash";

interface CrackTimeScenario {
...
export interface PassphraseDictionaryStats {
  profile: PassphraseDictionaryProfile;
  label: string;
  size: number;
  bitsPerWord: number;
}
...
export function generatePassword(settings: PasswordSettings): string {
  ...
  for (let attempt = 0; attempt < 500; attempt += 1) {
    ...
    if (
      validatePasswordCandidate(candidate, selectedPools, settings.enforceMix) &&
  ...
  const fallback = shuffle(
    Array.from({ length: settings.length }, () => alphabet[randomIndex(alphabet.length)]),
  ).join("");
  return fallback;
}

export function estimatePasswordEntropy(settings: PasswordSettings): number {
  const pools = buildPools(settings);
...
export function estimatePassphraseEntropy(settings: PassphraseSettings): number {
...
export function analyzeSecret(secret: string, theoreticalEntropyBits?: number): SecretAssessment {
  ...
  if (length < 12) warnings.push("length below 12 characters");
  if (length >= 16) strengths.push("length 16+");
  if (uniqueRatio < 0.5) warnings.push("low character uniqueness");
  if (uniqueRatio > 0.75) strengths.push("high unique character ratio");

  const classes = countCharacterClasses(value);
  if (classes <= 1) warnings.push("single character class detected");
  if (classes >= 3) strengths.push("multiple character classes");

  const repeatRuns = countRepeatRuns(value, 3);
  if (repeatRuns > 0) warnings.push("contains repeated character runs");

  const sequentialRuns = countSequentialRuns(value, 3);
  if (sequentialRuns > 0) warnings.push("contains keyboard/alphabet/number sequences");

  const weakMatches = weakFragments.filter((fragment) => lower.includes(fragment));
  if (weakMatches.length > 0) warnings.push("contains common password fragments");
  if (/\b(19|20)\d{2}\b/.test(value)) warnings.push("contains a likely year");

  const entropyBits = theoreticalEntropyBits ?? estimateObservedEntropy(value);
  let penalty = 0;
  if (length < 8) penalty += 28;
  else if (length < 12) penalty += 18;
  else if (length < 16) penalty += 8;
  if (classes <= 1) penalty += 20;
  else if (classes === 2) penalty += 8;
  if (uniqueRatio < 0.5) penalty += 10;
  penalty += repeatRuns * 6;
  penalty += sequentialRuns * 7;
  penalty += weakMatches.length * 14;
  if (/^\d+$/.test(value)) penalty += 18;
...
export function gradeLabel(grade: SecretGrade): string {
...
export function getPassphraseDictionaryStats(profile: PassphraseDictionaryProfile): PassphraseDictionaryStats {
...
export function generatePasswordBatch(settings: PasswordSettings, count: number): CandidateRow[] {
...
export function generatePassphraseBatch(settings: PassphraseSettings, count: number): CandidateRow[] {
  const rows: CandidateRow[] = [];
  for (let i = 0; i < count; i += 1) {
...
function buildPools(settings: PasswordSettings): string[] {
  const pools: string[] = [];
  if (settings.upper) pools.push(uppercaseChars);
  if (settings.lower) pools.push(lowercaseChars);
  if (settings.digits) pools.push(digitChars);
  if (settings.symbols) pools.push(PASSWORD_SYMBOLS);
  if (!settings.avoidAmbiguity) return pools;
  return pools
    .map((pool) => Array.from(pool).filter((char) => !ambiguousChars.has(char)).join(""))
    .filter((pool) => pool.length > 0);
}

function validatePasswordCandidate(candidate: string, pools: string[], enforceMix: boolean): boolean {
  if (!enforceMix) return true;
  return pools.every((pool) => Array.from(candidate).some((char) => pool.includes(char)));
}

function hasMinUniqueChars(candidate: string, minUniqueChars: number): boolean {
  return new Set(candidate).size >= minUniqueChars;
}

function hasRepeatRun(value: string, minRun: number): boolean {
  return countRepeatRuns(value, minRun) > 0;
}

function countRepeatRuns(value: string, minRun: number): number {
  let runs = 0;
  let streak = 1;
...
function hasSequentialRun(value: string, minRun: number): boolean {
  return countSequentialRuns(value, minRun) > 0;
}

function countSequentialRuns(value: string, minRun: number): number {
  const lower = value.toLowerCase();
  let count = 0;
...
function applyCaseStyle(word: string, style: PassphraseCaseStyle): string {
  if (style === "lower") return word;
  if (style === "upper") return word.toUpperCase();
  if (style === "title") return `${word[0].toUpperCase()}${word.slice(1)}`;
  const mode = randomIndex(3);
  if (mode === 0) return word.toUpperCase();
  if (mode === 1) return `${word[0].toUpperCase()}${word.slice(1)}`;
  return word;
}

function dictionaryWord(profile: PassphraseDictionaryProfile, index: number): string {
  const segments = dictionarySegments[profile];
  let cursor = index;
...
function pickPassphraseWord(
  ...
  if (!usedWords || !usedWords.has(candidate)) return candidate;
  ...
  for (let attempts = 0; attempts < 128; attempts += 1) {
    ...
    if (!usedWords.has(candidate)) return candidate;
  }
...
function estimateObservedEntropy(value: string): number {
  let alphabet = 0;
  if (/[a-z]/.test(value)) alphabet += 26;
  if (/[A-Z]/.test(value)) alphabet += 26;
  if (/[0-9]/.test(value)) alphabet += 10;
  if (/[^A-Za-z0-9\s]/.test(value)) alphabet += PASSWORD_SYMBOLS.length;
  if (/\s/.test(value)) alphabet += 1;
...
function mapEntropyToGrade(bits: number): SecretGrade {
  if (bits < 40) return "critical";
