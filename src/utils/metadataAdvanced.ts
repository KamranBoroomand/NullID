...
import { INPUT_LIMITS, readFileBytesWithLimit } from "./inputLimits.js";

export type MetadataRiskLevel = "low" | "medium" | "high";
type MetadataTargetKind = "image" | "document" | "video" | "archive" | "unknown";
...
interface MetadataReviewSection {
...
export async function sanitizePdfMetadata(file: File | Blob): Promise<PdfSanitizeResult> {
  const bytes = await readFileBytesWithLimit(file, { label: "PDF metadata file", maxBytes: INPUT_LIMITS.metadataFileBytes });
