...
export function VaultView({ onOpenGuide }: VaultViewProps) {
  ...
  const [keyHintProfiles] = usePersistentState<KeyHintProfile[]>(
  ...
  const beginVaultImport = useCallback(
    async (file?: File | null, mode: "plain" | "encrypted" = "plain") => {
      ...
      try {
        ...
        if (mode === "plain") {
          const parsed = JSON.parse(await readFileTextWithLimit(file, { label: "Vault snapshot", maxBytes: INPUT_LIMITS.jsonImportBytes })) as unknown;
