...
export function SafeShareView({ onOpenGuide }: SafeShareViewProps) {
  ...
  useEffect(() => {
    ...
    setTextPreview(applySanitizeRules(textInput, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware));
    void applySanitizeRulesAsync(textInput, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware, { signal: controller.signal })
      .then((next) => {
        if (!controller.signal.aborted) setTextPreview(next);
      })
      .catch((error) => {
  ...
  const handleTextFile = useCallback(
    async (file?: File | null) => {
      ...
      try {
        setMode("text");
        setTextInput(await readFileTextWithLimit(file, {
          label: "Safe Share text",
  ...
  const handleShareFile = useCallback(
    async (file?: File | null) => {
      ...
      try {
        const bytes = await readFileBytesWithLimit(file, { label: "Safe Share file", maxBytes: INPUT_LIMITS.metadataFileBytes });
  ...
  const buildPreviewPackage = useCallback(async () => {
    if (mode === "text") {
      ...
      if (!textPreview.commitAllowed) {
        throw new Error(textPreview.customDiagnostics[0] ?? "Custom regex rules must complete before export");
      }
    ...
    sourceBytes,
    textInput,
    textPreview.commitAllowed,
    textPreview.customDiagnostics,
    textSourceLabel,
  ]);
    ...
    <div className="workspace-scroll">
      ...
      <section className="panel" aria-label={tr("Safe Share overview")}>
        ...
        <div className="controls-row" style={{ alignItems: "center" }}>
          ...
          {previewPackage ? <Chip label={protectAtExport ? tr("NULLID:ENC:2 at export") : tr("unsigned package")} tone={protectAtExport ? "accent" : "muted"} /> : null}
        </div>
      </section>
        ...
        <section className="panel" aria-label={tr("Safe share export settings")}>
          ...
          <label className="microcopy">
            <input
              ...
              style={{ marginRight: "0.45rem" }}
            />
            {tr("Wrap the exported package in a NULLID:ENC:2 envelope")}
          </label>
          {protectAtExport ? (
            <input
              className="input"
          ...
          <ul className="microcopy">
            ...
            <li>{tr("NULLID:ENC:2 adds confidentiality and AES-GCM integrity for the exported file, not sender identity.")}</li>
      ...
      <section className="panel" aria-label={tr("Safe share export")}>
        ...
        <div className="controls-row">
          <button className="button" type="button" onClick={() => void exportPackage()} disabled={isExporting || !previewPackage || (mode === "text" && !textPreview.commitAllowed)}>
