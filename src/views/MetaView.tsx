...
import { readBrowserImageDimensions, sanitizeBrowserImage } from "../utils/metadataCleaning.js";
...
export function MetaView({ onOpenGuide }: MetaViewProps) {
    ...
    async (file: File) => {
      try {
        ...
        const cleaned = await sanitizeBrowserImage(file, {
          scale: resizePercent / 100,
          outputSupport: supportedOutput,
          outputChoice,
          quality: quality / 100,
        });
  ...
  const handleAdvancedFile = useCallback(async (file?: File | null) => {
    ...
    try {
      ...
      const analysisBytes = shouldReadFull
        ? await readFileBytesWithLimit(file, { label: "Metadata file", maxBytes: INPUT_LIMITS.metadataFileBytes })
        : await readHeadTailBytes(file, 1_000_000);
      ...
      if (analysis.format === "zip") {
        const archiveBytes = shouldReadFull
          ? analysisBytes
          : await readFileBytesWithLimit(file, { label: "Archive file", maxBytes: INPUT_LIMITS.metadataFileBytes });
  ...
  const handleArchiveManifestFile = useCallback(async (file?: File | null) => {
    ...
    try {
      const text = await readFileTextWithLimit(file, {
        ...
        maxBytes: INPUT_LIMITS.jsonImportBytes,
      ...
      <div className="grid-two">
        <div className="panel" aria-label={tr("Metadata input")}>
          ...
          >
            ...
            <div className="microcopy">{tr("jpeg / png / webp / gif / bmp")}</div>
          </div>
...
async function readHeadTailBytes(file: File, sliceSize: number) {
  if (file.size <= sliceSize * 2) {
    return readFileBytesWithLimit(file, { label: "Metadata sample", maxBytes: INPUT_LIMITS.metadataFileBytes });
  }
...
async function readImageDimensions(file: File) {
  return readBrowserImageDimensions(file, 1);
}
