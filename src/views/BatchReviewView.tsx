...
import { INPUT_LIMITS, assertFileBatchWithinLimit, readFileBytesWithLimit } from "../utils/inputLimits.js";
...
export function BatchReviewView({ onOpenGuide, onSelectModule }: BatchReviewViewProps) {
  ...
  const handleFiles = async (list?: FileList | null) => {
    ...
    try {
      ...
      assertFileBatchWithinLimit(files, {
        label: "Batch review",
      ...
      const nextItems = await Promise.all(
        files.map(async (file) =>
          createBatchFileReviewItem({
            ...
            sourceBytes: await readFileBytesWithLimit(file, { label: file.name, maxBytes: INPUT_LIMITS.batchFileBytes }),
            enabledRuleSets: ruleSetState,
          })),
      );
      ...
      console.error(error);
      push(error instanceof Error ? error.message : "file analysis failed", "danger");
    } finally {
