...
import { applySanitizeRules, applySanitizeRulesAsync, type PolicyPack } from "../utils/sanitizeEngine.js";
...
import { INPUT_LIMITS, readFileBytesWithLimit, readFileTextWithLimit } from "../utils/inputLimits.js";
...
export function IncidentWorkflowView({ onOpenGuide }: IncidentWorkflowViewProps) {
  ...
  useEffect(() => {
    ...
    setNotesPreview(applySanitizeRules(notesText, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware));
    void applySanitizeRulesAsync(notesText, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware, { signal: controller.signal })
      .then((next) => {
        if (!controller.signal.aborted) setNotesPreview(next);
      })
      .catch((error) => {
  ...
  const [draftTextPreview, setDraftTextPreview] = useState(() =>
    applySanitizeRules(textArtifactInput, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware),
  );
  useEffect(() => {
    const controller = new AbortController();
    setDraftTextPreview(applySanitizeRules(textArtifactInput, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware));
    void applySanitizeRulesAsync(textArtifactInput, textPolicy.rulesState, textPolicy.customRules, textPolicy.jsonAware, { signal: controller.signal })
      .then((next) => {
        if (!controller.signal.aborted) setDraftTextPreview(next);
      })
      .catch((error) => {
        if (controller.signal.aborted) return;
        console.error(error);
        setDraftTextPreview({
          output: textArtifactInput,
          applied: [],
          report: [error instanceof Error ? error.message : "custom regex worker failed"],
          linesAffected: 0,
          customStatus: "failed",
  ...
  const handleDraftFile = useCallback(
    async (file?: File | null) => {
      ...
      try {
        const bytes = await readFileBytesWithLimit(file, { label: "Incident file", maxBytes: INPUT_LIMITS.metadataFileBytes });
  ...
  const handleTextArtifactFile = useCallback(
    async (file?: File | null) => {
      if (!file) return;
      try {
        setTextArtifactInput(await readFileTextWithLimit(file, {
          label: "Incident text artifact",
          maxBytes: INPUT_LIMITS.textFileBytes,
  ...
  const addTextArtifact = () => {
    ...
    if (!draftTextPreview.commitAllowed) {
      push("custom regex rules must complete before adding artifact", "danger");
      return;
    }
  ...
  const buildPreview = useCallback(async () => {
    ...
    if (notesText.trim()) {
      if (!notesPreview.commitAllowed) {
        throw new Error(notesPreview.customDiagnostics[0] ?? "Custom regex rules must complete before export");
      }
    ...
    modeId,
    notesText,
    notesPreview.commitAllowed,
    notesPreview.customDiagnostics,
    producer,
    protectAtExport,
    purpose,
    recipientScope,
    ...
    <div className="workspace-scroll">
      ...
      <section className="panel" aria-label={tr("Incident workflow overview")}>
        ...
        <div className="controls-row" style={{ alignItems: "center" }}>
          ...
          <Chip label={protectAtExport ? tr("NULLID:ENC:2 at export") : tr("unsigned package")} tone={protectAtExport ? "accent" : "muted"} />
        </div>
      </section>
      ...
      <div className="grid-two">
        ...
        <section className="panel" aria-label={tr("Incident mode")}>
          ...
          <ul className="microcopy">
            {mode.guidance.map((line, index) => (
              <li key={`guidance:${index}:${line}`}>{tr(line)}</li>
            ))}
          </ul>
          ...
          <label className="microcopy">
            <input
              ...
              style={{ marginRight: "0.45rem" }}
            />
            {tr("Wrap the exported incident package in a NULLID:ENC:2 envelope")}
          </label>
          {protectAtExport ? (
            <input
              className="input"
      ...
      <div className="grid-two">
        <section className="panel" aria-label={tr("Case notes")}>
          ...
          <ul className="microcopy">
            {notesFindings.length > 0
              ? notesFindings.map((entry, index) => <li key={`${entry.label}:${entry.count}:${index}`}>{tr(entry.label)}: {entry.count}</li>)
        ...
        <section className="panel" aria-label={tr("Additional artifacts")}>
          ...
          />
          <div className="controls-row">
            <button className="button" type="button" onClick={addTextArtifact} disabled={textArtifactInput.trim().length > 0 && !draftTextPreview.commitAllowed}>
          ...
          {textArtifactInput.trim() ? (
            <ul className="microcopy">
              {draftTextFindings.length > 0
                ? draftTextFindings.map((entry, index) => <li key={`${entry.label}:${entry.count}:${index}`}>{tr(entry.label)}: {entry.count}</li>)
          ...
          {draftCleanActions.length ? (
            <>
              ...
              <ul className="microcopy">
                {draftCleanActions.map((line, index) => (
                  <li key={`clean-action:${index}:${line}`}>{tr(line)}</li>
                ))}
              </ul>
            </>
          ) : null}
            ...
            <>
              ...
              <table className="table">
                <tbody>
                  ...
                  <tr>
                    <th>{tr("Protection")}</th>
                    <td>{protectAtExport ? tr("NULLID:ENC:2 envelope at export") : tr("plain workflow package JSON")}</td>
                  </tr>
                </tbody>
              </table>
              <ul className="microcopy">
                {previewPackage.summary.highlights.map((line, index) => (
                  <li key={`highlight:${index}:${line}`}>{tr(line)}</li>
                ))}
              </ul>
            </>
          ) : (
        ...
        <section className="panel" aria-label={tr("Incident explainability")}>
          ...
          {previewPackage?.report ? (
            <>
              ...
              <ul className="microcopy">
                {previewPackage.report.includedArtifacts.map((line, index) => (
                  <li key={`included:${index}:${line}`}>{localizeWorkflowLine(line, tr)}</li>
                ))}
              </ul>
              <div className="panel-subtext">{tr("What the receiver can verify")}</div>
              <ul className="microcopy">
                {previewPackage.report.receiverCanVerify.map((line, index) => (
                  <li key={`can-verify:${index}:${line}`}>{localizeWorkflowLine(line, tr)}</li>
                ))}
              </ul>
              <div className="panel-subtext">{tr("What the receiver cannot verify")}</div>
              <ul className="microcopy">
                {previewPackage.report.receiverCannotVerify.map((line, index) => (
                  <li key={`cannot-verify:${index}:${line}`}>{localizeWorkflowLine(line, tr)}</li>
                ))}
              </ul>
            </>
          ) : (
      ...
      <div className="grid-two">
        <section className="panel" aria-label={tr("Warnings and limitations")}>
          ...
          <ul className="microcopy">
            {previewPackage ? (
              [...previewPackage.warnings, ...previewPackage.limitations].map((line, index) => <li key={`notice:${index}:${line}`}>{tr(line)}</li>)
            ) : (
              ...
              <div key={section.id}>
                ...
                <ul className="microcopy">
                  {section.items.map((line, index) => (
                    <li key={`${section.id}:${index}:${line}`}>{localizeWorkflowLine(line, tr)}</li>
                  ))}
                </ul>
              </div>
            ))}
      ...
      <section className="panel" aria-label={tr("Incident export")}>
        ...
        <div className="controls-row">
          <button className="button" type="button" onClick={() => void exportPackage()} disabled={isExporting || !previewPackage || !notesPreview.commitAllowed}>
