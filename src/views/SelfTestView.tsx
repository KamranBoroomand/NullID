...
import { SELF_TEST_INTERVAL_SECONDS, clampSelfTestIntervalSeconds } from "../utils/selfTestSettings";
...
export function SelfTestView({ onOpenGuide }: SelfTestViewProps) {
  ...
  const [monitorIntervalSec, setMonitorIntervalSec] = usePersistentState<number>(
    "nullid:selftest:interval",
    SELF_TEST_INTERVAL_SECONDS.defaultValue,
  );
  ...
  useEffect(() => {
    ...
    const intervalMs = clampSelfTestIntervalSeconds(monitorIntervalSec) * 1000;
      ...
      <div className="panel">
        ...
        <div className="controls-row">
          ...
          <input
            className="input"
            type="number"
            min={SELF_TEST_INTERVAL_SECONDS.min}
            max={SELF_TEST_INTERVAL_SECONDS.max}
            value={monitorIntervalSec}
            onChange={(event) => setMonitorIntervalSec(clampSelfTestIntervalSeconds(Number(event.target.value)))}
