...
export function RedactView({ onOpenGuide }: RedactViewProps) {
  ...
  const [findings, setFindings] = useState(() => scanRedaction(input, activeDetectors, customRules, { minimumSeverity, minTokenLength }));
  useEffect(() => {
    const controller = new AbortController();
    setOutput("");
    setFindings(scanRedaction(input, activeDetectors, customRules, { minimumSeverity, minTokenLength }));
    void scanRedactionAsync(input, activeDetectors, customRules, { minimumSeverity, minTokenLength }, { signal: controller.signal })
      .then((next) => {
        if (!controller.signal.aborted) setFindings(next);
      })
      .catch((error) => {
  ...
  const canCommitRedaction = findings.commitAllowed && input.length > 0;
  ...
  const applyCustomRule = async () => {
    ...
    const validation = await validateCustomRegexRule({ pattern: customPattern, flags: "gi", replacement: "" });
    if (!validation.ok) {
      push("invalid regex", "danger");
      return;
    }
    setCustomRules((prev) => [...prev, { label: customLabel || "custom", pattern: customPattern, flags: "gi" }]);
    setCustomPattern("");
    push("custom rule added", "accent");
  };

  const handleApply = () => {
    if (!canCommitRedaction) {
      push("custom regex rules must complete before apply", "danger");
      return;
    }
    setOutput(redacted);
  ...
  const handleCopy = async () => {
    if (!canCommitRedaction) {
      push("custom regex rules must complete before copy", "danger");
      return;
    }
  ...
  const handleDownload = () => {
    if (!canCommitRedaction) {
      push("custom regex rules must complete before download", "danger");
      return;
    }
  ...
  const exportFindingsReport = () => {
    ...
    const payload = {
      ...
      matches: previewChanges.slice(0, 400).map((change) => ({
        ...
        originalLength: change.original.length,
        redactedPreview: change.replacement,
    ...
    <div className="workspace-scroll">
      ...
      <div className="grid-two">
        ...
        <div className="panel" aria-label={tr("Redaction output")}>
          ...
          <div className="controls-row">
            <button className="button" type="button" onClick={handleApply} disabled={!canCommitRedaction}>
              {tr("apply redaction")}
            </button>
            <button className="button" type="button" onClick={handleCopy} disabled={!canCommitRedaction}>
              {tr("copy")}
            </button>
            <button className="button" type="button" onClick={handleDownload} disabled={!canCommitRedaction}>
              {tr("download")}
            </button>
            <button className="button" type="button" onClick={exportFindingsReport} disabled={!canCommitRedaction}>
              {tr("export report")}
            </button>
          </div>
          <div className="status-line">
            ...
            <span className="microcopy">{tr("custom regex")} {tr(findings.customStatus)}</span>
          </div>
          <div className="status-line">
