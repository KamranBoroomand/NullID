...
function AppShell() {
  ...
  const [layoutMode, setLayoutMode] = usePersistentState<LayoutMode>(
    { key: "nullid:layout-mode", validator: isLayoutMode },
    DEFAULT_LAYOUT_MODE,
  );
  ...
  }, [closeProfileExportDialog, profileExportKeyHint, profileExportPassphrase, profileExportSign, push, tr]);
  ...
  const saveProfileHint = useCallback(() => {
    ...
    setProfileExportKeyHint(findKeyHintProfileById(result.profiles, result.selectedId)?.keyHint ?? profileExportKeyHint);
    setProfileExportError(null);
  ...
  }, [closeProfileImportDialog, pendingProfileImportFile, profileImportDescriptor, profileImportPassphrase, push, tr]);
  ...
  }, [push, setActiveModule, tr, wipeConfirmText, wipeIncludeVault]);
  ...
  const useNavDrawer = isForcedMobile ? true : isForcedDesktop ? false : viewport.width < 1040;
  const isCompact = isForcedDesktop ? false : viewport.width < 840;
  ...
  return (
    <div
      className={`app-surface ${isCompact ? "is-compact" : ""} ${isShort ? "is-short" : ""}`}
    ...
    >
      ...
      <ActionDialog
        ...
        description={tr("Export supported non-sensitive preferences and configuration as JSON. Optional HMAC metadata can be verified during import with the same passphrase.")}
...
function clearManagedLocalState(options: { includeVault: boolean }) {
  ...
  for (let index = 0; index < localStorage.length; index += 1) {
    ...
    if (key.startsWith("nullid:vault:data:")) continue;
