...
export const guideTools: GuideToolContent[] = [
  ...
  },
  {
    ...
    whatItDoes:
      "Guided local workflow for preparing text snippets or files to share safely, with workflow presets, sanitize or metadata review, optional NULLID:ENC:2 wrapping, and receiver-friendly workflow package export.",
    whatWhen: [
    ...
    howSteps: [
      ...
      "Optionally wrap the exported workflow package in a NULLID:ENC:2 envelope before sending it onward.",
    ],
    limits: [
      "Workflow packages are still unsigned in this step; they do not prove sender identity.",
      "NULLID:ENC:2 adds confidentiality for the exported file, not public-key authenticity.",
  ...
  },
  {
    ...
    limits: [
      ...
      "NULLID:ENC:2 only protects the exported file at rest/in transit for parties with the passphrase; it is not a sender signature.",
  ...
  },
  {
    ...
    whatWhen: [
      ...
      "Summarize what kinds of sensitive content appear in a pasted message, export, or transcript without external API calls or analytics",
    ],
    howSteps: [
      "Paste text or load a local text file.",
  ...
  },
  {
    ...
    whatItDoes:
      "Receiver-side inspection for workflow packages, safe-share bundles, policy packs, profile snapshots, vault snapshots, and NULLID:ENC:2 envelopes with honest trust labels.",
    whatWhen: [
    ...
    howSteps: [
      ...
      "Provide an envelope passphrase only when the package is wrapped in NULLID:ENC:2.",
