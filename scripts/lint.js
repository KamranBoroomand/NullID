  ...
  function visit(node) {
    if (ts.isStringLiteralLike(node) && URL_RE.test(node.text)) {
      ...
      if (callName && NETWORK_CALLS.has(callName)) {
        if (!isApprovedServiceWorkerFetch(filePath, node, callName)) {
...
function isApprovedServiceWorkerFetch(filePath, node, callName) {
  if (callName !== "fetch" || path.relative(ROOT, filePath).replace(/\\/g, "/") !== "public/sw.js") {
    return false;
  }
  const [firstArg] = node.arguments;
  if (!firstArg) return false;
  if (ts.isIdentifier(firstArg) && ["manifestUrl", "request"].includes(firstArg.text)) return true;
  if (
    ts.isPropertyAccessExpression(firstArg) &&
    ts.isIdentifier(firstArg.expression) &&
    firstArg.expression.text === "entry" &&
    firstArg.name.text === "url"
  ) {
    return true;
  }
  if (ts.isCallExpression(firstArg) && ts.isIdentifier(firstArg.expression) && firstArg.expression.text === "scopedUrl") {
    return true;
  }
  return false;
}
